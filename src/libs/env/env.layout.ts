"server-only";
import { z } from "zod";

// ✅ Load environment variables from .env file
// ✅ Define schema with defaults and transformations
const envConfigSchema = z.object({
  NEXT_PUBLIC_ROOT_LAYOUT_API: z
    .url()
    .trim()
    .default("http://localhost:3000/api/layout"),
  NEXT_PUBLIC_HOMEPAGE_PRODUCTS_API: z
    .url()
    .trim()
    .default("http://localhost:3000/api/products"),
});

// ✅ Validate process.env safely
const parsed = envConfigSchema.safeParse(process.env);

if (!parsed.success) {
  throw new Error(
    `❌ Invalid layout environment variables:\n${parsed.error.issues
      .map((i) => `• ${i.path.join(".")}: ${i.message}`)
      .join("\n")}`
  );
}

// ✅ Export validated config
export const envRootLayoutConfig = Object.freeze({
  ROOT_LAYOUT_API: parsed.data.NEXT_PUBLIC_ROOT_LAYOUT_API,
  HOMEPAGE_PRODUCTS_API: parsed.data.NEXT_PUBLIC_HOMEPAGE_PRODUCTS_API,
});

// ✅ Optional: Export type
export type EnvRootLayoutConfig = z.infer<typeof envConfigSchema>;
